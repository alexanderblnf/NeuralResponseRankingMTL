...
class DMN_CNN_MTL_All(BasicModel):
    def __init__(self, config):
        super(DMN_CNN_MTL_All, self).__init__(config)
        self.__name = 'DMN_CNN_MTL_All'
        ...
        if not self.check():
            raise TypeError('[DMN_CNN_MTL_All] parameter check wrong')
        print '[DMN_CNN_MTL_All] init done'
    ...
    def build(self):
        ...
        for i in range(self.config['text1_max_utt_num']):
            ...
            cross1 = Match(match_type='dot')([q_embed, d_embed])
            # dot product of GRU output representations
            cross2 = Match(match_type='dot')([q_rep, d_rep])
            ...
            # MLP  50 is the setting in Wu et al in ACL'17
        ...
        out_web = Dense(1)(accum_stack_gru_hidden_flat_drop)
        model_web = Model(inputs=[query, doc], outputs=out_web)
        return model, model_web, model_clf
