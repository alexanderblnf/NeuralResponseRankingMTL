...
class DMN_CNN_MTL_Web_v2(BasicModel):
    def __init__(self, config):
        super(DMN_CNN_MTL_Web_v2, self).__init__(config)
        self.__name = 'DMN_CNN_MTL_Web_v2'
        ...
        if not self.check():
            raise TypeError('[DMN_CNN_MTL_Web_v2] parameter check wrong')
        print '[DMN_CNN_MTL_Web_v2] init done'
    ...
    def build(self):
        ...
        doc_web = Input(name='doc', shape=(self.config['text2_maxlen_web'],))
        show_layer_info('Input doc', doc_web)

        embedding = Embedding(self.config['vocab_size'], self.config['embed_size'], weights=[self.config['embed']], trainable = self.embed_trainable)
        d_embed = embedding(doc)
        show_layer_info('Doc Embedding', d_embed)

        d_embed_web = embedding(doc_web)
        show_layer_info('Doc Embedding Web', d_embed_web)
        accum_stack = []
        accum_stack_web = []
        ...
        for i in range(self.config['text1_max_utt_num']):
            ...
            d_rep_web = Bidirectional(
                GRU(self.config['hidden_size'], return_sequences=True, dropout=self.config['dropout_rate']))(d_embed_web)
            show_layer_info('Bidirectional-GRU-Web', d_rep_web)
            ...
            cross1_web = Match(match_type='dot')([q_embed, d_embed_web])  # dot product of embeddings (Can try other interaction functions here such as cosine, ind, bi-linear, etc.)
            cross2_web = Match(match_type='dot')([q_rep, d_rep_web])  # dot product of GRU output representations

            cross = Lambda(concate)([cross1, cross2])
            cross_web = Lambda(concate)([cross1_web, cross2_web])
            z = Reshape((self.config['text1_maxlen'], self.config['text2_maxlen'], 2))(cross) # batch_size * t1_len * t2_len * 2 channels
            z_web = Reshape((self.config['text1_maxlen'], self.config['text2_maxlen_web'], 2))(cross_web) # batch_size * t1_len * t2_len * 2 channels
            ...
            for j in range(self.config['num_conv2d_layers']):
                ...
                z_web = Conv2D(filters=self.config['2d_kernel_counts'][j], kernel_size=self.config['2d_kernel_sizes'][j],
                           padding='valid', activation='relu')(z_web)
                show_layer_info('Conv2D', z_web)
                z_web = MaxPooling2D(pool_size=(self.config['2d_mpool_sizes'][j][0], self.config['2d_mpool_sizes'][j][1]))(
                    z_web)
                show_layer_info('MaxPooling2D', z_web)
            ...
            z_web = Flatten()(z_web)
            show_layer_info('Flatten-z', z_web)
            z_web = Dense(50, activation="tanh")(z_web)  # MLP  50 is the setting in Wu et al in ACL'17
            show_layer_info('Dense-z', z_web)
            accum_stack_web.append(z_web)


        accum_stack = Lambda(stack)(accum_stack) # batch_size * max_turn_num * 50
        show_layer_info('accum_stack', accum_stack)

        accum_stack_web = Lambda(stack)(accum_stack_web)  # batch_size * max_turn_num * 50
        show_layer_info('accum_stack', accum_stack_web)
        ...
        accum_stack_gru_hidden_web = Bidirectional(
            GRU(self.config['hidden_size'], return_sequences=True, dropout=self.config['dropout_rate']))(accum_stack_web)
        show_layer_info('accum_stack_gru_hidden', accum_stack_gru_hidden_web)
        accum_stack_gru_hidden_flat_web = Reshape((-1,))(accum_stack_gru_hidden_web)
        show_layer_info('accum_stack_gru_hidden_flat', accum_stack_gru_hidden_flat_web)
        accum_stack_gru_hidden_flat_drop_web = Dropout(rate=self.config['dropout_rate'])(accum_stack_gru_hidden_flat_web)
        show_layer_info('Dropout', accum_stack_gru_hidden_flat_drop_web)
        ...
        out_web = Dense(1)(accum_stack_gru_hidden_flat_drop_web)
        model_web = Model(inputs=[query, doc_web], outputs=out_web)
