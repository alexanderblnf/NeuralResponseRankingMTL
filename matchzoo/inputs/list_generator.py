...
class DMN_ListGenerator_Web_v2(ListBasicGenerator):
    def __init__(self, config={}):
        super(DMN_ListGenerator_Web_v2, self).__init__(config=config)
        self.data1 = config['data1']
        self.data2 = config['data2']
        self.data1_maxlen = config['text1_maxlen']
        self.data1_max_utt_num = int(config['text1_max_utt_num'])
        ...
        self.check_list.extend(['data1', 'data2', 'text1_maxlen', 'text2_maxlen', 'embed', 'text1_max_utt_num'])
        if not self.check():
            raise TypeError('[DMN_ListGenerator_Web_v2] parameter check wrong.')
        print '[DMN_ListGenerator_Web_v2] init done, list number: %d. ' % (self.num_list)
