...
class DMN_PairGeneratorWithIntents(PairBasicGenerator):
    def __init__(self, config):
        super(DMN_PairGeneratorWithIntents, self).__init__(config=config)
        self.__name = 'DMN_PairGeneratorWithIntents'
        self.config = config
        self.data1 = config['data1']
        self.data2 = config['data2']
        self.data1_maxlen = config['text1_maxlen']
        ...
        if not self.check():
            raise TypeError('[DMN_PairGeneratorWithIntents] parameter check wrong.')
    ...
    def get_batch_static(self):
        ...
        for i in range(self.batch_size * 2):
            ...
            d2p_len = min(self.data2_maxlen, len(d2p_ws))
            X2[i, :d2p_len], X2_len[i] = d2p_ws[:d2p_len], d2p_len
            ...
            for j in range(utt_start, len(self.data1[d1])):
                d1_ws = self.data1[d1][j].split()
                d1_len = min(self.data1_maxlen, len(d1_ws))
                X1[i, j - utt_start, :d1_len], X1_len[i, j - utt_start] = d1_ws[:d1_len], d1_len
...
class DMN_PairGenerator_Web_v2(PairBasicGenerator):
    def __init__(self, config):
        super(DMN_PairGenerator_Web_v2, self).__init__(config=config)
        self.__name = 'DMN_PairGenerator_Web_v2'
        self.config = config
        self.data1 = config['data1']
        self.data2 = config['data2']
        self.data1_maxlen = config['text1_maxlen']
        self.data1_max_utt_num = int(config['text1_max_utt_num'])
        self.data2_maxlen = config['text2_maxlen_web']
        self.fill_word = config['vocab_size'] - 1
        self.check_list.extend(['data1', 'data2', 'text1_maxlen', 'text2_maxlen_web', 'text1_max_utt_num'])
        if config['use_iter']:
            self.batch_iter = self.get_batch_iter()
        if not self.check():
            raise TypeError('[DMN_PairGenerator_Web_v2] parameter check wrong.')
        print '[DMN_PairGenerator_Web_v2] init done'
