...
class Preparation(object):
    ...
    * Relation File: this file records the relationship between two texts, each line containing the label and a pair of ids.
    ...
    def parse_line_dmn(self, line, delimiter='\t'):
        subs = line.split(delimiter)
        # print('subs: ', len(subs))
        if len(subs) < 3:
            raise ValueError('format of data file wrong, should be \'label,text1(mulitple utterances septerated by tab),text2\'.')
        else:
            return subs
    ...
    @staticmethod
    def save_corpus_dmn(file_path, corpus, delim='\t'):
        f = open(file_path, 'w')
        for qid, text in corpus.items():
